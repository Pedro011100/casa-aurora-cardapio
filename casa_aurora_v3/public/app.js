(() => {
  ...
  const loadCart = () => {
    try {
      const saved = JSON.parse(localStorage.getItem('casa-aurora-cart') || '[]');
      ...
      localStorage.removeItem('casa-aurora-cart');
      return [];
    }
  };
  const state = { category: 'todas', search: '', page: 1, pageSize: 8, cart: loadCart(), service: true };
  const $ = (selector) => document.querySelector(selector);
  const save = () => { localStorage.setItem('casa-aurora-cart', JSON.stringify(state.cart)); renderCart(); };
