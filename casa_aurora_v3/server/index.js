...
app.get("/api/health", async (_req, res) => {
  try {
    ...
    return res.json({ ok: true, database: "sqlite", file: "database/casa-aurora.db", products: row.count });
  } catch (error) {
...
server.listen(port, host, () => console.log(`Casa Aurora JS + SQLite running on http://${host}:${port}`));
