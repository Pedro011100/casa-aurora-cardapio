...
A aplicação usa **SQLite local**. O banco é criado automaticamente em `database/casa-aurora.db` quando o servidor inicia. O schema é criado em `server/db.js` e está documentado em `schema.sqlite.sql`.
